...
if [ "$FULL_BUILD" = true ]; then
    # フルビルド用のコマンド
    RIGHT_BUILD_CMD="west build -p -s app -d build/right -b seeeduino_xiao_ble -S studio-rpc-usb-uart -- -DZMK_CONFIG=/workspaces/zmk-config/config -DSHIELD=moNa2_R -DZMK_EXTRA_MODULES=\"/workspaces/zmk-modules/zmk-pmw3610-driver;/workspaces/zmk-modules/zmk-listeners\""
    LEFT_BUILD_CMD="west build -p -s app -d build/left  -b seeeduino_xiao_ble -- -DZMK_CONFIG=/workspaces/zmk-config/config -DSHIELD=moNa2_L -DZMK_EXTRA_MODULES=/workspaces/zmk-modules/zmk-listeners"
...
if [ $right_status -eq 0 ] && [ $left_status -eq 0 ] && [ $reset_status -eq 0 ]; then
    ...
    cp build/right/zephyr/zmk.uf2 "$OUTPUT_DIR/moNa2_R-seeeduino_xiao_ble-zmk.uf2"
    cp build/left/zephyr/zmk.uf2 "$OUTPUT_DIR/moNa2_L-seeeduino_xiao_ble-zmk.uf2"
    ...
    echo -e "${BLUE}  └── Right:${NC} moNa2_R-seeeduino_xiao_ble-zmk.uf2"
    echo -e "${BLUE}  └── Left:${NC}  moNa2_L-seeeduino_xiao_ble-zmk.uf2"
